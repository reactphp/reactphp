...
The `React\Socket\Client\Connector` provides a single promise-based
`create($host, $ip)` method which resolves as soon as the connection
succeeds or fails.

```php
$connector = new React\Socket\Client\Connector($loop, $dns);
...
```php
$secureConnector = new React\Socket\Client\SecureConnector($connector, $loop);

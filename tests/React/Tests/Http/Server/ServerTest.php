...
use React\Http\Server\Server;
use React\Tests\Socket\Server\TestCase;
use React\Tests\Socket\Server\Stub\ServerStub;
use React\Tests\Socket\Server\Stub\ConnectionStub;
...
class ServerTest extends TestCase
{
    ...
    public function testRequestEvent()
    {
        ...
        $server->on('request', function ($request, $response) use (&$i) {
            ...
            $this->assertInstanceOf('React\Http\Server\Request', $request);
            $this->assertSame('/', $request->getPath());
            $this->assertSame('GET', $request->getMethod());
            $this->assertSame('127.0.0.1', $request->remoteAddress);

            $this->assertInstanceOf('React\Http\Server\Response', $response);
        });
